...
<body>
        ...
        <div id="my-dumps" class="uuper">
            <div style="display: flex; justify-content: space-between;">
                ...
                <div style="position: relative; width: 300px; font-family: Arial, sans-serif;">
                    <button id="dropdownBtn"
                        style="width: 100%; padding: 12px; background-color: #0c182f; border: 1px solid #1f3660; border-radius: 5px; color: white; text-align: left; font-size: 14px; cursor: pointer;">
                        Select Columns
                    </button>
                    <div id="dropdownMenu"
                        style="display: none; position: absolute; top: 100%; left: 0; width: 100%; background-color: white; border: 1px solid #ccc; border-radius: 5px; padding: 10px; box-sizing: border-box; z-index: 999; max-height: 300px; overflow-y: auto;">
            ...
            <div class="main-tbl321"
                        ...
                        <tr id="dump-<?php echo htmlspecialchars($dump['id']); ?>">
                            ...
                            <td style="padding: 10px; display: flex; justify-content: center; align-items: center;">
                                <button class="copy-button"
                                    ...
                                    onclick="copyDumpInfo(<?php echo json_encode($dump['id']); ?>)">Copy</button>
                                <button id="check-dump-button-<?php echo htmlspecialchars($dump['id']); ?>"
                                ...
                                <a type="button" onclick="deleteRow(<?php echo json_encode($dump['id']); ?>)"
                                    id="clear-btn-<?php echo $dump['id']; ?>" class="btn text-center btn-with-icon"
                ...
                <div id="bottom-scrollbar"
                    style="overflow-x: auto; overflow-y: hidden; width: 100%; border: 1px solid #ddd; margin-top: 5px;">
    ...
    <div id="rules-popup2" class="popup-modal" style="display: none;">
        <div class="popup-content">
            ...
            <p>• For a smooth checking experience, please check one dump at a time. After receiving a response from the
                checker—either
                <span class="live">LIVE</span> or <span class="dead">DEAD</span>—you can proceed to check the remaining
                dumps.
            </p>

            <p>• Checking a dump may, in most cases, '<span class="kill">KILL</span>' the dump. If the dump's status
                results in a
                <span class="live">LIVE</span> response, you will not be eligible for a refund. Therefore, please do not
                request a refund for a dump with a
                <span class="live">LIVE</span> status.
            </p>

            <p>• The fee for checking a dump is $0.50. If the dump's result is <span class="dead">DEAD</span>, you will
                receive a full refund for the dump amount, along with the credit for checking the dump.</p>

            <p>• For <span class="live">LIVE</span> dumps, you will not receive a refund for the checking fee.</p>

            <p>• We use the most advanced third-party checking systems. If a dump's status is returned as <span
                    class="live">LIVE</span>, we will not issue a refund. This checking service is not part of our
                default system, so we rely on and trust the third-party providers we use to verify the dumps.</p>

            <p><b>• Do not attempt to abuse the dump checker in an effort to find bugs or exploit the system. We have
                    mechanisms in place that notify us when a user tries to use the checker in any way other than
                    intended. In such cases, we reserve the right to ban the user without prior notice, and any
                    deposited funds will be forfeited.</b></p>

            <p>• We reserve the right to disable the checker at any time without prior notice in the event of
                malfunctions or misuse.</p>

            <p>• You have the option to delete your entire dump activity log by clicking the 'Delete Dumps Activity
                Logs' button, if you prefer not to keep a record of previously checked dumps. You also have the option
                to delete purchased and used dumps individually by clicking the 'Delete' button next to each dump.</p>
