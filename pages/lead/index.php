...
<div class="main-content">
    <div id="leads" class="uuper">
        ...
        <div style="position: relative; width: 100%; max-width: 400px; margin: 0 auto; display: flex; align-items: center; justify-content: center;">
            <input type="text" id="searchBar" placeholder="Search Leads..."
...
function fetchFiles(currentPage = 1, searchQuery = '') {
    fetch(`get_files.php?section=Leads&page=${currentPage}&search=${encodeURIComponent(searchQuery)}`)
        .then(response => {
            if (!response.ok) {
        ...
        .then(data => {
            ...
            if (data.files && data.files.length > 0) {
                ...
                gridContainer.innerHTML = '<p>No Leads found.</p>';
                document.getElementById("pagination").innerHTML = '';
            }
        })
        .catch(error => {
            console.error('Error fetching files:', error);
            // alert('No files available in the Leads section.');
        });
}
