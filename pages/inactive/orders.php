...
function fetchOrders(page = 1) {
    fetch(`get_orders.php?page=${page}`)
        ...
        .catch(error => {
            ...
            const ordersContainer = document.getElementById('orders-container');
            ordersContainer.innerHTML = '<p>This account is inactive. Please load your balance.</p>';
        });
}
...
function fetchOrders(page = 1) {
    fetch(`get_orders.php?page=${page}`)
        ...
        .then(data => {
            // Check for account inactive status
            if (data.inactive) {
                alert("This account is inactive. Please load your balance.");
                window.location.href = '<?= $urlval?>pages/inactive/index.php';
                return;
            }

            // Render orders if the account is active
            const ordersContainer = document.getElementById('orders-container');
            ordersContainer.innerHTML = ''; // Clear existing content
