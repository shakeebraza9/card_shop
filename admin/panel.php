...
try {
    ...
    $totalCardsSold = $pdo->query("SELECT COUNT(*) FROM credit_cards WHERE status = 'sold'")->fetchColumn();
...
.kpi-box {
 ...
 @keyframes borderGlow {
      ...
      25% { box-shadow: 0 0 5px #ec4899, 0 0 10px #ec4899; border-color: #ec4899; }
      50% { box-shadow: 0 0 5px #f59e0b, 0 0 10px #f59e0b; border-color: #f59e0b; }
      75% { box-shadow: 0 0 5px #10b981, 0 0 10px #10b981; border-color: #10b981; }
...
<body>
  <div class="container">
    ...
    <div class="flex-container">
      ...
      <div class="flex-item kpi-box" style="border-left: 4px solid rgb(255, 145, 200); <?php if($unreadMessages > 0) echo 'animation: borderGlow 4s infinite;'; ?>">
  <h3>Support Messages</h3>
  <p><?php echo $unreadMessages;; ?> unread 
  <a href="sc.php" style="color: #ec4899; font-weight: bold; margin-left: 8px; font-size: medium; text-decoration: none;">REPLY</a> </p>
</div>
    ...
    <div class="flex-container">
      <div class="flex-item kpi-box width: auto;">
        ...
        <div style="display: flex; justify-content: space-between; padding: 8px 0; border-bottom: 1px solid #e5e7eb;">
          <span style="font-size: 14px; font-weight: 600;"><?php echo htmlspecialchars($user['username']); ?></span>
...
<div class="grid-container" style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 20px; margin: auto; margin-bottom: 24px;">
  <?php foreach ($statCategories as $stat): ?>
    <div class="kpi-box" style="background: #fff; padding: 20px; border-radius: 8px; box-shadow: 0 2px 5px rgba(0,0,0,0.08);">
  ...
  <?php endforeach; ?>
    ...
    <div style="margin-bottom: 24px;">
      ...
      <div id="tab-all" class="tab-content active">
        ...
        <div style="padding: 16px; border-bottom: 1px solid #e5e7eb; display: flex; align-items: center;">
          <div style="width: 40px; height: 40px; border-radius: 50%; background-color: #eef2ff; display: flex; align-items: center; justify-content: center; margin-right: 16px;">
            <span style="color: #4f46e5; font-weight: 600;"><?php echo strtoupper(substr($activity['item_type'] ?? '', 0, 2)); ?></span>
          </div>
          <div style="flex: 1;">
            <p style="margin: 0; font-size: 14px;">
              <span style="font-weight: 600;"><?php echo htmlspecialchars($activity['item_type'] ?? ''); ?></span> - 
              <?php echo htmlspecialchars($activity['buy_itm'] ?? ''); ?> for $<?php echo htmlspecialchars($activity['item_price'] ?? ''); ?> by 
              <span style="color: #4f46e5;"><?php echo htmlspecialchars($activity['user_name'] ?? ''); ?></span>
            </p>
            <p style="margin: 4px 0 0 0; font-size: 12px; color: #6b7280;"><?php echo timeAgo($activity['created_at'] ?? ''); ?></p>
          </div>
        </div>
        <?php endforeach; ?>
      </div>
      <div id="tab-card" class="tab-content">
        ...
        <div style="padding: 16px; border-bottom: 1px solid #e5e7eb;">
          <p style="margin: 0; font-size: 14px;">
            Card #: <strong><?php echo htmlspecialchars($log['card_number'] ?? ''); ?></strong> - Status: <?php echo htmlspecialchars($log['status'] ?? ''); ?>
          </p>
          <p style="margin: 4px 0 0 0; font-size: 12px; color: #6b7280;"><?php echo timeAgo($log['date_checked'] ?? ''); ?></p>
        </div>
        <?php endforeach; ?>
      </div>
      <div id="tab-dump" class="tab-content">
        ...
        <div style="padding: 16px; border-bottom: 1px solid #e5e7eb;">
          <p style="margin: 0; font-size: 14px;">
            Track1: <strong><?php echo htmlspecialchars($log['track1'] ?? ''); ?></strong> - Status: <?php echo htmlspecialchars($log['status'] ?? ''); ?>
          </p>
          <p style="margin: 4px 0 0 0; font-size: 12px; color: #6b7280;"><?php echo timeAgo($log['date_checked'] ?? ''); ?></p>
        </div>
        <?php endforeach; ?>
    ...
    <!-- Footer -->
    <div style="margin-top: 40px; text-align: center; padding: 20px; color: #6b7280; font-size: 14px; border-top: 1px solid #e5e7eb;">
