        ...
        if (empty($errors)) {
            ...
            if (file_exists($uploadFile)) {
                ...
                if (move_uploaded_file($file['tmp_name'], $uploadFile)) {
                    ...
                    // Remove var_dump() from here
                    $stmt = $pdo->prepare("
                        INSERT INTO uploads (name, description, file_path, price, section)
                        VALUES (
                            AES_ENCRYPT(?, ?),
                            AES_ENCRYPT(?, ?),
                            AES_ENCRYPT(?, ?),
                    ...
                    $_SESSION['uploaded_file_name'] = $formData['name'];
                    $_SESSION['uploaded_file_price'] = $formData['price'];
...
if (isset($_SESSION['success_message'])): ?>
    <div style="padding: 15px; margin-bottom: 20px; border-radius: 4px; background-color: rgba(46, 204, 113, 0.1); border-left: 4px solid #2ecc71; color: #27ae60; transition: opacity 0.5s ease;">
        <p style="margin: 5px 0;"><?php echo htmlspecialchars($_SESSION['success_message']); ?></p>
    </div>
    <?php 
